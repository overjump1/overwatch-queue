...
public final class MQTT {
    ...
    private init(host: String, port: Int, username: String? = nil, password: String? = nil, will: Will? = nil, optionsDict: [OptionsKey: Any]? = nil) {
        self.host = host
        self.port = port
...
public extension MQTT {
    ...
    convenience init(host: String, port: Int, username: String, password: String, will: Will? = nil, options: [OptionsKey: Any]? = nil) {
        self.init(host: host, port: port, username: username, password: password, will: will, optionsDict: options)
    }
    ...
    func publish(to topicName: String, qos: QoS = .mostOnce, retain: Bool = false, message: Data? = nil, completion: ((Result<Void, PublishError>) -> Void)? = nil) {
        transportQueue.async {
            self.sendPublish(topicName, qos:qos, retain: retain, message: message, completion: completion)
        }
    }
...
private extension MQTT {
    ...
    func sendPublish(_ topicName: String, qos: QoS, retain: Bool = false, message: Data?, completion: ((Result<Void, PublishError>) -> Void)? = nil) {
        let packetId = (qos != .mostOnce) ? nextPacketId() : nil
        let packet = try! PublishPacket(topicName: topicName, qos: qos, packetId: packetId, retain: retain, message: message)
        if qos != .mostOnce {
