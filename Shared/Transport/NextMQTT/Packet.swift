...
struct ConnectFlags: OptionSet, MQTTEncodable {
    ...
    static let will              = ConnectFlags(rawValue: 1 << 2)
    static let willRetain        = ConnectFlags(rawValue: 1 << 5)
    static let username          = ConnectFlags(rawValue: 1 << 7)
    static let password          = ConnectFlags(rawValue: 1 << 6)

    static func willQoS(_ qos: MQTT.QoS) -> Self {
        ConnectFlags(rawValue: qos.rawValue << 3)
    }
}

public extension MQTT {
    /// The Last Will and Testament the broker publishes on this client's behalf if the
    /// connection drops without a clean DISCONNECT — see section 3.1.2.5 of the spec.
    struct Will {
        let topic: String
        let message: String
        let qos: MQTT.QoS
        ...
        public init(topic: String, message: String, qos: MQTT.QoS = .mostOnce, retain: Bool = false) {
            self.topic = topic
...
struct ConnectPacket: EncodablePacket {
    ...
    // Payload
    let clientId: String
    // Will Properties (section 3.1.3.2) — always zero-length (no delay/format/expiry props).
    let willPropLength: UInt8?
    let willTopic: String?
    let willPayload: String?
    let username: String?
    let password: String?

    init(clientId: String, username: String? = nil, password: String? = nil, keepAlive: UInt16 = 10, cleanStart: Bool = false, sessionExpiry: UInt32 = 0, will: MQTT.Will? = nil) throws {
        let propLength: UInt8 = (sessionExpiry > 0) ? 5 : 0
        let variableHeaderLength = MQTT.ProtocolName.byteCount + 1 + 1 + 2 + 1 + UInt(propLength)
        let willLength: UInt = will.map { 1 + $0.topic.byteCount + $0.message.byteCount } ?? 0
        let payloadlength = clientId.byteCount + willLength + (username?.byteCount ?? 0) + (password?.byteCount ?? 0)
        ...
        if let will {
            self.willPropLength = 0
            self.willTopic = will.topic
            self.willPayload = will.message
        } else {
            self.willPropLength = nil
            self.willTopic = nil
            self.willPayload = nil
        }
        ...
        if let will {
            connectFlags.insert(.will)                // section 3.1.2.5 Will Flag
            connectFlags.insert(.willQoS(will.qos))    // section 3.1.2.6 Will QoS
            if will.retain {
                connectFlags.insert(.willRetain)       // section 3.1.2.7 Will Retain
            }
        }
        if username != nil {
...
struct PublishPacket: CodablePacket {
    ...
    init(topicName: String, qos: MQTT.QoS, packetId: UInt16? = nil, retain: Bool = false, message: Data?) throws {
        ...
        var controlOptions: ControlOptions = [.packetType(.publish), .qos(qos)]
        if retain { controlOptions.insert(.retain) }
        self.fixedHeader = FixedHeader(controlOptions: controlOptions, remainingLength: remainingLength)
